...
### 좋아하는 YouTube 채널을 오프라인 podcast 로 듣기
...
4. 내 폰 podcast 앱이 표준 RSS 로 **자동 fetch**
...
```
  ...
  [오디오 추출]  m4a 파일로 보관
       │
       ▼  내 집 네트워크에서 RSS 발행 (`/feed.xml`)
  [Podcast 앱]  🎧 표준 RSS 로 자동 fetch
```

> 도구: `yt-dlp` + Flask 콘솔(:8001) + 정적 RSS 서버(:8000), macOS LaunchAgent 로 항상 실행.
> ⚠️ 폰 podcast 앱에서 실제 구독·재생은 K 가 본인 기기로 확인하는 단계 — 본 데모에는 미포함.
...
- 큐레이션 → 후보 10개 자동 제시
- 다운로드 검증한 3편 (실제 m4a 파일 + `feed.xml` `<item>` 노출 확인):
  - 실밸개발자 채널 — *"안드레이 카파시가 말하는 AI-native 의 역량"* (24m)
  - 보다 채널 — *"호르무즈 해협 …"* (28m)
  - 지식인사이드 채널 — *"AI로봇 근황 ㅣ지식선발대 EP.1"* (19m)
- 정적 서버가 LAN IP 로 `feed.xml` + m4a Range 응답 정상 (HTTP 200)

> 실제 podcast 앱 구독·재생, 실사용·운영 데이터는 미검증. 오늘 만든 결과 그대로.
